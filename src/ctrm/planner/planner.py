"""template class of multi-agent path planner
Author: Keisuke Okumura
Affiliation: TokyoTech & OSX

Note: the class includes single-agent path planner (space-time A*)
"""
...
from ..environment import Instance, continuous_collide_spheres
...
class Planner(metaclass=ABCMeta):
    ...
    Attributes:
        ...
        solution (list[list[TimedNode]]): paths
        ...
        time_static_collide (float): elapsed time for vertex collision check
        cnt_continuous_collide (int): number of edge collision check
        time_continuous_collide (float): elapsed time for edge collision check
        lowlevel_expanded (int): number of expanded node
        lowlevel_explored (int): number of explored node
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.trms: list[TimedRoadmap] = trms
        self.verbose: int = verbose
        self.solved: bool = False
        self.solution: list[list[TimedNode]] = []
    ...
    def solve(self) -> Result:
        ...
        if self.solved:
            return Result(
                ...
                sum_of_travel_dists=self.get_sum_of_travel_dists(
                    self.solution
                ),
                maximum_travel_dists=self.get_maximum_travel_dists(
                    self.solution
                ),
                cnt_static_collide=self.cnt_static_collide,
                cnt_continuous_collide=self.cnt_continuous_collide,
                elapsed_static_collide=self.time_static_collide,
                elapsed_continuous_collide=self.time_continuous_collide,
    ...
    def validate(self) -> bool:
        ...
        for t in range(1, T + 1):
            for i in range(self.ins.num_agents):
                ...
                # check continuity
                if (
                    np.linalg.norm(loc_i_t - loc_i_t_prev)
                    > self.ins.max_speeds[i]
                ):
                    logger.error(
                        f"invalid move at t={t-1}:"
                        f"{loc_i_t_prev} -> {loc_i_t}"
                    )
                    return False
                # check collisions
    ...
    def collide_dynamic_agents(
        ...
        res, elapsed = self.__collide_dynamic_agents(
    ...
    def __collide_dynamic_agents(
        ...
        # accurate check
        return continuous_collide_spheres(from1, to1, rad1, from2, to2, rad2,)
    ...
    def collide_static_agents(
        ...
        res, elapsed = self.__collide_static_agents(pos1, size1, pos2, size2)
    ...
    def get_single_agent_path(
        self,
        agent: int,
        check_fin: Callable[[TimedNode], bool],
        insert: Callable[[TimedNode, list[list]], None],
        valid_successor: Callable[[TimedNode, TimedNode], bool],
        **kwargs,
        ...
        check_fin (Callable[[TimedNode], bool]): goal checker
        insert (Callable[[TimedNode, list[list]], None]):
        ...
        **kwargs:
        ...
        Returns:
            Optional[list[TimedNode]]: path or None
        """
        # A* search
        trm = self.trms[agent]
    ...
    def get_costs_metric(
        self, paths: list[list[TimedNode]], fn: Callable
    ) -> float:
        """compute cost metric
        ...
        return fn(
            [
                get_cost(path, self.ins.goals[i], self.ins.goal_rads[i],)
                for i, path in enumerate(paths)
            ]
        )

    def get_sum_of_costs(self, paths: list[list[TimedNode]]) -> float:
        """sum-of-costs"""
        return self.get_costs_metric(paths, sum)

    def get_maximum_costs(self, paths: list[list[TimedNode]]) -> float:
        """aka. makespan"""
        return self.get_costs_metric(paths, max)

    def get_travel_dists_metric(
        self, paths: list[list[TimedNode]], fn: Callable
    ):
    ...
    def get_sum_of_travel_dists(self, paths: list[list[TimedNode]]) -> float:
        """sum of travel distances"""
        return self.get_travel_dists_metric(paths, sum)

    def get_maximum_travel_dists(self, paths: list[list[TimedNode]]) -> float:
